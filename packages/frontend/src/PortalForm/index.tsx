...
const portalSizeValid = (size: PortalSize | null) =>
  size !== null && ['blue', 'yellow', 'royal', 'const'].includes(size)

const formValidator = (
  from: Zone,
  to: Zone,
  portalSize: string | null,
  hours: number | null,
  minutes: number | null
): InputError[] => {
  ...
  } else if (portalSize !== 'royal' && portalSize !== 'const') {
    if (!minutes && !hours) {
...
const MappingBar = () => {
  ...
  const [lastZone, setLastZone] = useState<Zone>(DEFAULT_ZONE)
  ...
  useEffect(() => {
    ...
    } else if (!timeLeft || size === 'royal' || size === 'const') {
      setHours(null)
      setMinutes(null)
    ...
    (e: FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        if (
          ...
          (hr + min > 0 || portalSize === 'royal' || portalSize === 'const')
        ) {
          addPortal({
  ...
  const gotoLastAddedZone = useCallback(
    (zone: Zone) => {
      dispatch({
        type: PortalMapActionTypes.CENTER,
        centerZone: zone,
      })
    },
    [dispatch]
    ...
    <form onSubmit={handleSubmit} autoComplete="off">
      <div className={styles.mappingBar}>
          ...
          <FormControl fullWidth component="fieldset">
            ...
            <div className={styles.flexColumn}>
              <TextField
                ...
                disabled={portalSize === 'royal' || portalSize === 'const'}
                id="time-hour"
        ...
        <div className={styles.row}>
          <Button
            style={{
              backgroundColor: '#317c06',
            }}
            onClick={() => gotoLastAddedZone(lastZone)}
          >
            Go to last added zone
          </Button>
        </div>
      </div>
    </form>
